...
from allocation_system.utils import clear_folder, select_processing_month
from allocation_system.main_after import main_process


def main():
    print("部門別セグメント配賦プログラム実行")
    print("収益認識基準 - 修正後の実績に対して配賦処理を行います。")
    print("修正前後でプログラムの起動に誤りがある場合、処理月入力待機中にコンソールを終了してください。")

    # ------------------------------------ 定数宣言 ------------------------------------
    EXP_NO_BMN_SEG_REC = "9997"
    ...
    try:
        ...
        clear_folder(os.path.join(TEMP_FOLDER, "csv"), LOGGER=LOGGER)
        clear_folder(os.path.join(TEMP_FOLDER, "logs"), LOGGER=LOGGER)
        clear_folder(os.path.join(OUT_PUT_FOLDER, "csv"), LOGGER=LOGGER)
        ...
        bmn_master_sql = f"""
            SELECT SumKbn, GCode, NCode, LongName, TEndDate
        ...
        try:
            ...
            LOGGER.error("部門マスタの取得に失敗しました", exc_info=True)
        ...
        try:
            ...
            LOGGER.error("セグメントマスタの取得に失敗しました", exc_info=True)
        ...
        try:
            ...
            LOGGER.error("勘定科目マスタの取得に失敗しました", exc_info=True)
        ...
        if all(df is not None and not df.empty for df in dataframes):
            ...
            journal_df = main_process(
                ...
                bmn_seg_record_df, hif_information_df,
                den_date, START_YM, END_YM,
                LOGGER=LOGGER
            )
        else:
