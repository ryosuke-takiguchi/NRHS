...
def perform_allocation(
    ...
    HKGO = 999888
    swk_add_rows = []  # ここに仕訳データを貯めて最後にDataFrameにする
    ...
    for kmk_cd in target_kmk_list:
        for seg_cd in jitsuzai_segs_list:
            ...
            if not haihutaisyou[6] == 0 and haihutaisyou[7] == 0:
                amount_to_allocate = haihutaisyou[6] - haihutaisyou[7]
                swk_add_rows.append([
                    "910" + str(row_dict['パターンＮＯ']).zfill(3), str(HKGO), "", "", "", "", str(kmk_cd), str(bmn_cd).zfill(4),
                    str(seg_cd).zfill(4), "40", str(amount_to_allocate), tekiyo
                ])
                ...
                for i, target_bmn in enumerate(target_bmn_list):
                    rate = hif_kjn_df.loc[hif_kjn_df["配賦先部門コード"] == target_bmn, "配賦係数"].iloc[0]
                    ...
                    swk_add_rows.append([
                        "910" + str(row_dict['パターンＮＯ']).zfill(3), str(kmk_cd), str(target_bmn).zfill(4), str(seg_cd).zfill(4), "40",
                        str(kingaku), str(HKGO), "", "", "", "", tekiyo
                    ])
